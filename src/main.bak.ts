...
function main(): void {
    ...
    const ctx = setupContext(gl, brush, camera);
...
function getWebGl(): WebGL2RenderingContext {
...
function setupContext(
    ...
    // uniforms
    paint.uniforms.proj = rp_paint.makeProjMat(
        mat3.create(),
        settings.paintExtent,
    );
    paint.uniforms.view = rp_paint.makeViewMat(
        mat3.create(),
        settings.paintExtent,
    );
    paint.uniforms.brush = brush;

    present.uniforms.camera = camera;
...
function draw(gl: WebGL2RenderingContext, ctx: Context) {
